...
```text
//流程
用户 -> 签收消息 -> 后端更新用户的签收时间，判断新消息的最旧时间（签收ids中最小的时间） >= 当前连续时间（用户签收的连续时间）。
...
public void acknowledgeMessages(Long groupId, Long userId, List<String> messageIds) {
    ...
    // 判断是否是连续确认
    if(isContinuousAck(receipt, messages)) {
        receipt.setLastContinuousTime(maxTime);
    }
...
private boolean isContinuousAck(MessageReceipt receipt, List<GroupMessage> messages) {
    // 获取消息中最旧的时间
    Instant minMessageTime = messages.stream()
        .map(GroupMessage::getCreatedAt)
        .min(Instant::compareTo)
        .orElse(Instant.MAX);
        
    // 连续确认条件：新消息的最旧时间 >= 当前连续时间
    return !minMessageTime.isBefore(receipt.getLastContinuousTime());
}
```
...
public List<GroupMessage> getUnreadMessages(Long groupId, Long userId) {
    ...
    // 
    return filterAckedMessages(candidates, receipt.getAcknowledgedIds());
}
```
