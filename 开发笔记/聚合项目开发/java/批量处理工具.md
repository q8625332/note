...
* @param batchOperation 批处理操作，返回本次处理的数量  
     ...
     * @return 批处理结果  
     */  
    public static BatchResult executeBatch(Supplier<Integer> batchOperation, BatchConfig config) {  
        ...
        try {  
            do {  
                ...
                while (retryCount <= config.getMaxRetries()) {  
                    try {  
                        currentBatchProcessed = batchOperation.get();  
                        break; // 成功执行，跳出重试循环  
                    } catch (Exception e) {  
                        retryCount++;  
...
config.getOperationName(), totalProcessed, executeCount - 1);  
     ...
     * 使用默认配置执行批处理  
     */  
    public static BatchResult executeBatch(Supplier<Integer> batchOperation) {  
        return executeBatch(batchOperation, new BatchConfig());  
    }  
    /**  
     * 使用指定批次大小执行批处理  
     */  
    public static BatchResult executeBatch(Supplier<Integer> batchOperation, int batchSize) {  
        return executeBatch(batchOperation, new BatchConfig().batchSize(batchSize));  
    }  
    /**  
     * 使用指定批次大小和操作名称执行批处理  
     */  
    public static BatchResult executeBatch(Supplier<Integer> batchOperation, int batchSize, String operationName) {  
