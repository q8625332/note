...
dtoMap.forEach((k, v) -> {  
    ...
    itemMap.forEach((k2, v2) -> {  
        ...
        item.setOutsourcingReceiveId(0L);  
        item.setOutsourcingReceiveNo("");  
        ...
        item.setCylinderNo(firstItemData.getCylinderNo());  
        item.setWeavePartnerId(0L);  
        item.setWeavePartnerName("");  
        item.setSupplierId(partner.getId());  
        item.setSupplierName(partner.getName());  
        item.setSupplierType(partner.getType());  
        item.setRefundQuantity(Optional.ofNullable(firstItemData.getBoltQuantity()).orElse(0L));  
        item.setRefundQuantityOfMeter(Optional.ofNullable(firstItemData.getQuantity()).map(m -> UnitUtils.yardToMeter(m, 2)).orElse(BigDecimal.ZERO));  
        item.setRefundQuantityOfYard(Optional.ofNullable(firstItemData.getQuantity()).orElse(BigDecimal.ZERO));  
        item.setInWarehouseOfMeter(item.getRefundQuantityOfMeter());  
        item.setInWarehouseOfYard(item.getRefundQuantityOfYard());  
